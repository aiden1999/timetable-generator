...
A version of a genetic algorithm was implemented by Perzina (date?) and was
applied to a timetabling problem with real world data.
...
hard and soft constraints. The authors' algorithm performed best against other 
implementations of Tabu search, and  was also compared to other algorithms that did not 
use Tabu search at all. The authors' implementation performed better than all 
the other algorithms, except variable neighbourhood search on the largest 
dataset, and simulated annealing which performed better across all dataset
sizes.
...
timetabling problem is a discrete combinatorial optimisation problem, the
authors modified the algorithm.
...
When testing their algorithm, the authors (who?) used input data consisting of 
...
\renewcommand\bibname{References}  % #TODO: add the rest of the references
