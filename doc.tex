...
Banbara et al used Answer Set Programming to solve the timetabling problem.
...
violation, whereas constraints that have a calculated cost attatched to them
...
\begin{itemize}
	...
	\item \( S_5 \) \textbf{RoomStability}: All lectures of a course should be
		...
		The penalty points, reflecting the number of distinct room but the
		firest, are imposed on each violation.
	...
	\item \( S_7 \) \textbf{TravelDistance}: Students should have the time to
		...
		For a given curriculum we acount for a violation every time there is an
		instaneous move: two lectures in rooms located in different buildings in
		two adjacent periods within the same day.
		Each instaneous move in a curriculum counts as 1 violation.
	\item \( S_8 \) \textbf{RoomSuitability}: Some rooms may not be suitable for
		a given course becuase of the absence of necessary equipment.
	...
	\item \( S_9 \) \textbf{DoubleLectures}: Some courses require that lectures
		...
		than one lecture in one day, a lecture non-grouped to aother is not
		allowed.
...
defined as specific set of soft constraints togther with weights associated with
each of them.
...
constaint \( S_9 \) would not be used.
Formally, the timetabling problem is formulaated as a combinatorial optimisation
...
\subsection{Bat Inspired Algorithm}  % #TODO: bat inpired algorithm

\section{Comparison of Methods}  % #TODO: comparison of methods

\section{Chosen Method}  % #TODO: chosen method

\chapter{Requirements}  % #TODO: requirements
...
\section{Aspirational Requirements}  % #TODO: aspirational requirements
...
\subsection{Selection}  % #TDOD: selection
...
\renewcommand\bibname{References}  % #TDOD: add the rest of the refercnes
\bibliography{references.bib}

% Include appendix section if needed
%\include{Appendix/Appendix}  % #TODO: add source code and diagrams?
